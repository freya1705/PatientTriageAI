...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_decorations(self, page_count):
        ...
        self.drawString(36, 22, "Confidential — Prototype Submission Document &bull; Evaluated on 20 Synthetic Scenarios &bull; 51 Automated Tests")
        self.setStrokeColor(colors.HexColor("#e2e8f0"))
        self.setLineWidth(0.5)
...
def build_pdf():
    pdf_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "PatientTriage_AI_Accenture_Submission_README.pdf")
    ...
    story.append(Paragraph("Active Autonomous Emergency Department Safety Control Tower &bull; Accenture Innovation Challenge 2026", subtitle_style))
    ...
    meta_data = [
        [
            ...
            Paragraph("<b>VERIFICATION</b><br/>51/51 Tests Passed (100%)", body_style),
    ...
    # 1. Project Overview
    story.append(Paragraph("1. Project Overview &amp; Control Tower Concept", h1_style))
    story.append(Paragraph(
        "<b>PatientTriage.ai</b> is an active autonomous emergency department clinical decision-support control tower. "
        "Traditional emergency triage operates as a static, one-time snapshot at intake. PatientTriage.ai continuously tracks "
        "vital sign trajectory velocity (&Delta;SpO₂, &Delta;HR), observation shelf-life (Safety Clocks), data uncertainty (Unknown &ne; Safe), "
        "and clinician attention allocation (The Attention Gap). "
        "It provides continuous decision support, while deterministic guardrails protect patient safety and clinicians retain 100% decision authority.",
        body_style
    ))
    story.append(Paragraph(
        "The system continuously answers: <b>1. Who needs attention right now? &bull; 2. Why are they becoming unsafe? &bull; 3. What is the exact next clinical action?</b>",
        callout_style
    ))

    # 2. Core Problem & EHR Moat
    story.append(Paragraph("2. The Core Problem &amp; Competitive Moat (Vs. Legacy EHR Scores)", h1_style))
    story.append(Paragraph("• <b>1. Silent Waiting Room Deterioration:</b> ESI Level 3/4 patients worsen unmonitored; physiological decline may remain undetected until a subsequent reassessment or clinical deterioration becomes apparent.", bullet_style))
    ...
    story.append(Paragraph("3. System Architecture &amp; 8 Core Intelligence Engines", h1_style))
    arch_data = [
        [Paragraph("<b>Tier</b>", body_style), Paragraph("<b>Responsibilities &amp; Scope</b>", body_style), Paragraph("<b>Key Modules</b>", body_style)],
        [Paragraph("<b>Tier 1: Deterministic Guardrails</b>", body_style), Paragraph("Deterministic red-flags (SpO₂ &lt; 85%, SBP &lt; 75 mmHg, FAST Stroke, pediatric stridor) that bypass statistical models. Downgrade safety blocking.", body_style), Paragraph("<code>safety_guardrails.py</code><br/><code>downgrade_guard.py</code>", code_style)],
        [Paragraph("<b>Tier 2: AI &amp; Trajectory Engine</b>", body_style), Paragraph("Continuous vital velocity (&Delta;SpO₂, &Delta;HR), confidence decay, uncertainty scoring, Attention Gap re-ranking, 108 EMS Pre-Arrival Telemetry with LOINC FHIR parser, and Referral Eligibility Scoring (RES).", body_style), Paragraph("<code>risk_engine.py</code><br/><code>attention_gap_engine.py</code><br/><code>ems_routes.py</code>", code_style)],
        [Paragraph("<b>Tier 3: Multi-Surface Dispatch</b>", body_style), Paragraph("4 Workspaces: Overview (Control Tower), Nurse Tasks ('Next 5 Mins'), Floor Map (Chairs 1–20), and Lab Orders (Standing Pre-Orders). Unified slide-over Patient Drawer.", body_style), Paragraph("<code>PatientDrawer.jsx</code><br/><code>EDPressureMap.jsx</code>", code_style)],
    ]
    ...
    story.append(Paragraph("4. Core Intelligence Engines &amp; Attention Gap Formula", h1_style))
    ...
    story.append(Paragraph("<b>Parameter Calibration:</b> <code>w_r = 1.0</code> (base risk 0–100) &bull; <code>w_d = +25 to +40 pts</code> (&Delta;SpO₂ &le; -5% or &Delta;HR &ge; +20 bpm) &bull; <code>w_s = +20 to +35 pts</code> (safety expiry) &bull; <code>w_u = +15 to +25 pts</code> (missing vitals) &bull; <code>w_c = -35 pts</code> (discounts attended patients, elevating unattended cases to Rank #1).", body_style))
    ...
    story.append(Paragraph("Evaluated across 20 synthetic clinical scenarios and verified across 51 automated backend pytest cases:", body_style))
    ...
    eval_data = [
        ...
        [Paragraph("<b>Pre-Arrival Telemetry (108 EMS)</b>", body_style), Paragraph("<font color='#b91c1c'>Manual Handover Delay</font>", body_style), Paragraph("<font color='#047857'><b>Automated FHIR LOINC Ingestion</b></font>", body_style), Paragraph("<b>1-Click Bay Pre-Allocation</b>", body_style)],
    ...
    story.append(Paragraph("• <b>Phase 1 (Q3 2026):</b> Lab Validation &bull; 20 synthetic scenarios verified across 51 automated tests; sub-15ms inference latency.", bullet_style))
    ...
    story.append(Paragraph("• <b>Phase 3 (Q1–Q2 2027):</b> Live Pilot &bull; <b>Primary Safety Endpoint:</b> Reduction in Mean Time to Escalation (MTTE) (&gt;45% faster); <b>Operational Endpoint:</b> Nurse false-alarm rate &lt; 2 alerts/nurse/shift; <b>Economic Endpoint:</b> &ge;25% reduction in Left-Without-Being-Seen (LWBS).", bullet_style))
    story.append(Paragraph("• <b>Phase 4 (Q3 2027+):</b> Multi-Hospital Enterprise Network Scaling &bull; Regional dashboarding, telemedicine, and referral diversion.", bullet_style))

    # 7. Quick Start & Security
    story.append(Paragraph("7. Quick Start &amp; Security Architecture", h1_style))
    story.append(Paragraph("<code># Launch: .\\start.ps1 | Run 51 Automated Tests: python -m pytest -v | Web UI: http://localhost:5173</code>", code_style))
    story.append(Paragraph("• <b>Zero PHI:</b> 100% synthetic physiological datasets &bull; <b>Air-Gapped:</b> Zero external cloud LLM calls &bull; <b>Audit Ledger:</b> Append-only event logs.", bullet_style))

    # Regulatory Disclaimer
    story.append(Spacer(1, 4))
    disc_data = [[Paragraph("<b>Regulatory &amp; Safety Notice:</b> PatientTriage.ai is a clinical decision-support research prototype developed for the Accenture Innovation Challenge 2026. All patient cohorts are synthetically generated. This system is positioned under FDA Non-Device CDS (21 U.S.C. § 360aaa-1) and licensed clinicians retain 100% final override authority.", body_style)]]
